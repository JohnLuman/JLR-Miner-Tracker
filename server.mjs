...
function publicState() {
  ...
  return {
    app:{name:'JLR Miner Tracker',version:'2.8.4',systemCount:SYSTEM_DEFS.length,privacy:'Shared field state, system scan timestamps, and fleet-level mining totals only. Character location is read during Probe Scanner import; the character location itself is not retained.'},
...
server.listen(PORT,'0.0.0.0',()=>{console.log(`JLR Miner Tracker v2.8.4 listening on port ${PORT}`);console.log(`Website SSO: ${EVE_CLIENT_ID?'configured':'not configured'}`);console.log(`Tracked T3 systems: ${SYSTEM_DEFS.length}`)});

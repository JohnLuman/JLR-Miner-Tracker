...
function publicState() {
  ...
  return {
    app:{name:'JLR Miner Tracker',version:'2.9.68',systemCount:SYSTEM_DEFS.length,privacy:'Shared field state, system scan timestamps, and fleet-level mining totals only. Character location is read during Probe Scanner import; the character location itself is not retained.'},
...
function trackerSpokenSystem(system){
  ...
  return [...raw].map(ch=>ch==='-'?'tack':(digits[ch]||letters[ch]||ch)).join(' ');
}
...
async function syncAll({manual=false}={}){
  ...
  const startGapMs=manual?ESI_MANUAL_CHARACTER_START_GAP_MS:plan.startGapMs;
  syncInProgress=true;state.esi.lastError=null;broadcast();
  try{
    const results=await syncCharacters(entries,{forceMetadata:false,startGapMs,concurrency:plan.concurrency});
...
async function routeApi(req,res,url) {
  if(req.method==='GET'&&url.pathname==='/api/config')return json(res,200,{name:'JLR Miner Tracker',version:'2.9.68',ssoConfigured:Boolean(EVE_CLIENT_ID),callbackUrl:callbackUrl(req),publicUrl:requestBaseUrl(req),miningScope:MINING_SCOPE,skillsScope:SKILLS_SCOPE,fittingsScope:FITTINGS_SCOPE,assetsScope:ASSETS_SCOPE,locationScope:LOCATION_SCOPE,contactsScope:CONTACTS_SCOPE,corporationContactsScope:CORPORATION_CONTACTS_SCOPE,allianceContactsScope:ALLIANCE_CONTACTS_SCOPE,scopes:ESI_SCOPES,marketCharacterName:MARKET_CHARACTER_NAME});
  ...
  if(req.method==='POST'&&url.pathname==='/api/esi/sync'){
    ...
    if(syncInProgress)return json(res,202,{ok:true,alreadyRunning:true,scope:'all-connected-toons',queuedCharacters:connectedCharacters});
    syncAll({manual:true}).catch(console.error);
    return json(res,202,{ok:true,scope:'all-connected-toons',queuedCharacters:connectedCharacters});
  }
...
server.listen(PORT,'0.0.0.0',()=>{console.log(`JLR Miner Tracker v2.9.68 listening on port ${PORT}`);console.log(`Website SSO: ${EVE_CLIENT_ID?'configured':'not configured'}`);console.log(`Tracked T3 systems: ${SYSTEM_DEFS.length}`)});

...
assert.match(server,/TRACKER_INTEL_HOT_CACHE_MS = 30 \* 60 \* 1000/,'hot-zone snapshots refresh every 30 minutes');
...
assert.match(tracker,/persistent local cache/,'regional snapshots explain persistence');
assert.match(tracker,/automatic refresh every/,'regional snapshots explain 30-minute refresh');
...
assert.match(tracker,/Signal interference increased in/,'automatic interference increases can produce an Adam ping');
...
assert.match(tracker,/does not provide the live ESS bank balance/,'ESS UI does not falsely attribute client data to ESI');
assert.match(tracker,/not a public ESI field/,'interference UI does not falsely attribute client data to ESI');
...
assert.equal(pkg.version,'2.9.134','Tracker regional intel release is versioned');
assert.ok(index.includes('/tracker.css?v=2.9.134'),'browser loads Tracker intel CSS');
assert.ok(index.includes('/tracker.js?v=2.9.134'),'browser loads Tracker loader');
assert.ok(index.includes('/app.js?v=2.9.134'),'browser loads matching app release');

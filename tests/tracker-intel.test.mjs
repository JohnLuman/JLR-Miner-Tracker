...
assert.equal(pkg.version,'2.9.136','Hot Zones-only Tracker release is versioned');
assert.ok(index.includes('/tracker.css?v=2.9.136'),'browser loads Tracker intel CSS');
assert.ok(index.includes('/tracker.js?v=2.9.136'),'browser loads Tracker loader');
assert.ok(index.includes('/app.js?v=2.9.136'),'browser loads matching app release');
assert.match(trackerLoader,/tracker-core\.js\?v=2\.9\.136/,'Tracker core cache is busted');

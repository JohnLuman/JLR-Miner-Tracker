...
export function aggregateTrackedT3Ledger({rows,typeById,systemById,systemOreByName,priceByMineral,refineYield}){
  const daily=new Map();
  for(const row of rows||[]){
    const systemName=String(systemById?.[String(row?.solar_system_id)]?.name||'');
    const expectedFamily=systemOreByName?.[systemName];
    const variant=trackedT3OreVariant(expectedFamily,row?.type_id);
    if(!variant)continue;
